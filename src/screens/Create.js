...
import { Row, Col, Alert, Input, InputNumber, Card, Space, AutoComplete,
     Select, Button, Slider, Divider, Checkbox, Image, Spin, Modal, Tag, Layout, Typography} from 'antd';
...
export default function Create() {
  ...
  const onChange = (e) => {
    ...
    setToggleSave(e.target.checked)
  }
  ...
  const createGif = async () => {
    ...
    if(TAG.length === 0 || !category){
      alert('If you want to save the GIF please choose a category and enter tags.')
      return false;
    }
    ...
    if(!url){
      ...
      axios.post(`${process.env.REACT_APP_BACKEND}/convert`,
      {
        "url":url,
        "text": text || null,
        ...
        console.log(err)
        alert(`${err.event} , please try again`)
  ...
  const saveGif = async (url) => {
    if(!ToggleSave)
      return false;
    
    if(TAG.length === 0 || !category || !url){
      return false;
    }
    ...
    axios.post(`${process.env.REACT_APP_BACKEND}/savegif`,
      {
        'url': url,
        'tags': TAG,
        'category': category
  ...
  const deleteGif = async () => {
    axios.post(`${process.env.REACT_APP_BACKEND}/delete`, {
      url: gif
    }).then(res=>{
      console.log(res);
      if(res.status === 200){
      ...
      <Col  xs={22} sm={22} md={18} lg={18} xl={18}>
            ...
            />
              ...
              <div className='example' 
                style={{
                position:'absolute',
                zIndex:99999, left:-35, top:-35, 
                transform:'rotateY(0deg) rotate(-45deg)'}}> 
                <LottieAnimation lotti={camera} height={70} width={70} />
              </div>
                ...
                <Col key='col02' xs={24} sm={24} md={8} lg={8} xl={8}>
                  ...
                  <Card key='03'>
                      ...
                      <Input style={{ width: '75%' }} placeholder="enter text here..."
                    ...
                    <br />      
                      ...
                      <Title level={5}>GIF resolution</Title>
                        <Select  value={resolution} style={{width:'100%'}}  onChange={setResolution}>
                          { resList && 
                            resList.map((el, i) => {
                              return  <Option value={el} key={i}>{el}</Option>
                            })
                    ...
                    <Card key='05'>
                      ...
                      <Input
                        style={{width:'77%'}}
                        placeholder='Enter tags here (5 max)...'
                        //value={timeInput} 
                        value={tagInput}
                        onChange={e => {
                        ...
                        onPressEnter={e => {
                          ...
                          if(tags.length > 4 || !e.target.value){
                            return false;
                          }else{
                            let color = colors[random(1, tags.length -1 )]
                            addTags([...tags, {title: e.target.value, color}])
  ...
  <Footer style={{ textAlign: 'center' }}>
    <p>All right reserver (c) 2021 - Gifwif.com</p>
